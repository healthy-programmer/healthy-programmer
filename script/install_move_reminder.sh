...
echo "=== Move Reminder Installation Script (Linux) ==="
echo "This script will set up your environment for Move Reminder."
echo "You only need to run this once, or after changing your Python environment."
...
if ! $PYTHON -c "import tkcalendar" >/dev/null 2>&1; then
    ...
    if grep -q "externally-managed-environment" /tmp/tkcalendar_pip.log; then
        ...
        echo "Then run the run script using ~/move_reminder_venv/bin/python."
...
echo "Installation complete! You can now use run_move_reminder.sh to start the Move Reminder."