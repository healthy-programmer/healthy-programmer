...
#+BEGIN_SRC shell
./script/run_move_reminder.sh --interval 30 --duration 30 --position bottom-right
#+END_SRC

Or, for a quick test:

#+BEGIN_SRC shell
./script/run_move_reminder.sh --interval 1 --duration 15 --help
#+END_SRC
...
- `--interval INTERVAL`  
  Interval in minutes between reminders (default: 30)

- `--duration DURATION`  
  How long (seconds) to show the GIF window (default: 30)
